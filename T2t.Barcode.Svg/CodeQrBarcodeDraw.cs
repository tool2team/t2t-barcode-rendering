...
public class CodeQrBarcodeDraw : BarcodeDraw
{
    ...
    private class QRCodeEncoder
    {
        ...
        public virtual string Encode(string content, Encoding encoding)
        {
            ...
            {
                string.Format(RectTmpl, 0, 0, width, height, BackgroundColor)
            };
            ...
            for (int i = 0; i < matrix.Length; i++)
            {
                for (int j = 0; j < matrix.Length; j++)
                {
                    if (matrix[j][i])
                    {
                        rects.Add(string.Format(RectTmpl, j * _scale, i * _scale, _scale, _scale, ForegroundColor));
                    }
                }
            }
            return string.Format(SvgTmpl, matrix.Length * _scale + 1, matrix.Length * _scale + 1, string.Join('\n', rects));
        }
        ...
        {
            if (Code2dUtility.IsUnicode(content))
            {
    ...
    /// <returns></returns>
    protected virtual string DrawQr(string text, BarcodeMetricsQr metrics)
    {
