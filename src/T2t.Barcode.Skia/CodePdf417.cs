...
namespace T2t.Barcode.Skia
{
	...
	public class CodePdf417BarcodeDraw : BarcodeDraw
	{
		...
		private SKBitmap _outImage;
		private SKPointI _outPosition;
		...
		/// <returns></returns>
		public override SKBitmap Draw(string text, BarcodeMetrics metrics)
		{
			_text = Filter(text);
			return PaintCode();
		}
		...
		public virtual SKBitmap PaintCode()
		{
		...
		private void RenderCodeword(int codeword, int bits)
		{
			...
			{
				SKColor pixel = SKColors.White;
				if ((codeword & (2 ^ bit)) != 0)
				{
					pixel = SKColors.Black;
				}
				_outImage.SetPixel((int)_outPosition.X++, _outPosition.Y, pixel);
			}
		}
		...
		private SKBitmap PaintCodeInternal()
		{
			_bitColumns = START_CODE_SIZE * (_codeColumns + 3) + STOP_SIZE;
			_outImage = new SKBitmap(_bitColumns, _codeRows, SKColorType.Rgb888x, SKAlphaType.Opaque);
			int _codePtr = 0;
			...
			if ((_options & PdfRenderFlags.InvertBitmap) != 0)
			{
				...
				SKBitmap inverted = _outImage.Copy();
				for (int x = 0; x < inverted.Width; ++x)
				{
					for (int y = 0; y < inverted.Height; ++y)
					{
						SKColor color = inverted.GetPixel(x, y);
